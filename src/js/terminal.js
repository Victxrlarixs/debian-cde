...
function openMatrixTerminal(title = "ASCII") {
    ...
    closeBtn.onclick = () => { cancelAnimationFrame(animationId); win.remove(); };
    ...
    const chars = "ABCDE;FGHIJKLM$#!NOPQRS#$%^&*TUVWXYZ01#$%^&*23456789@#$%^&*()*&^%".split("");

    ctx.font = `${fontSize}px 'DejaVu Sans Mono', monospace`;

    function draw() {
        // Fondo negro semitransparente para efecto rastro
        ctx.fillStyle = "rgba(0, 0, 0, 0.1)";
        ctx.fillRect(0, 0, canvas.width, canvas.height);

        ctx.fillStyle = "#3b8f9b"; // verde clásico
        ...
        for (let i = 0; i < drops.length; i++) {
            ...
            if (drops[i] * fontSize > canvas.height && Math.random() > 0.975) {
                ...
                drops[i] += 0.8; // velocidad moderada para efecto Matrix
            }
        }
