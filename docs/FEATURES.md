...
document.querySelectorAll('#netscape .ns-menu-label').forEach((lbl) => {
  lbl.addEventListener('click', () => {
    ...
    // Close all menus
    document.querySelectorAll('#netscape .ns-menu.open')
  ...
  typeLine(prompt + step.command, () => {
    ...
    setTimeout(() => {
      ...
      if (stepIndex >= sequence.length) {
        ...
        const randomMsg = TRANSITION_MESSAGES[
...
function renderListView(path: string): void {
  ...
  let html = '<table class="fm-list"><thead><tr><th>Name</th><th>Size</th><th>Modified</th></tr></thead><tbody>';
